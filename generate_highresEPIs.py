...
saver = tf.train.import_meta_graph('model_-1to1/my_model50.meta')
saver.restore(sess, tf.train.latest_checkpoint('model_-1to1/.'))
...
for file in range(len(filelist_inputs)):
	...
	input_subEPIs = (input_subEPIs-127.5)/127.5
	output_subEPIs = (output_subEPIs-127.5)/127.5
...
for file in range(len(filelist_inputs)):
	...
	scipy.misc.toimage(output_img, cmax=1.0, cmin=0.0).save('output_-1to1/'+'output_'+filelist_outputs[file])
	scipy.misc.toimage(residual_img).save('residuals_-1to1/'+'residual_'+filelist_outputs[file])