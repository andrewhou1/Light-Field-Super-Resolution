...
saver = tf.train.import_meta_graph('model_Lab_space/my_model50.meta')
saver.restore(sess, tf.train.latest_checkpoint('model_Lab_space/.'))
...
for file in range(len(filelist_inputs)):
	...
	lab_input = color.rgb2lab(image)
	Lchannel =  lab_input[:, :, 0]
	...
	input_subEPIs[99, :, :, :] = np.reshape(Lchannel[:, startIndex-3:startIndex+colsRemaining], (17, 17, 1))
			
	output_filename = "Matlab_Training_Data/groundtruthblurred/groundtruthblurred/"+filelist_outputs[file]
	image = io.imread(output_filename)
	...
	output_subEPIs[99, :, :, :] = np.reshape(Lchannel[:, startIndex-3:startIndex+colsRemaining], (17, 17, 1))
	input_subEPIs = (input_subEPIs-50)/50
	output_subEPIs = (output_subEPIs-50)/50	
	...
	#scipy.misc.toimage(output_img, cmax=1.0, cmin=0.0).save('output_-1to1/'+'output_'+filelist_outputs[file])
	lab_EPI = np.zeros((17, 1400, 3))
	lab_EPI[:, :, 0] = output_img*50+50
	lab_EPI[:, :, 1] = lab_input[:, :, 1]
	lab_EPI[:, :, 2] = lab_input[:, :, 2]
	rgb_EPI = color.lab2rgb(lab_EPI)
	
	scipy.misc.toimage(rgb_EPI, cmax=1.0, cmin=0.0).save('output_Lab/'+'output_'+filelist_outputs[file])
	scipy.misc.toimage(residual_img).save('residuals_Lab/'+'residual_'+filelist_outputs[file])